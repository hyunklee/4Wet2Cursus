...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/01 15:36:50 by hyunklee          #+#    #+#             */
/*   Updated: 2021/06/18 16:36:32 by hyunklee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (1)
	{
		if (ft_strchr(backup, '\n') == -1)
		{
			...
			if (aend == -1)
				return (-1);
			buf[aend] = '\0';
			if (!(backup = ft_strjoin(backup, buf)))
				return (-1);
		}
		if ((cut = ft_strchr(backup, '\n')) != -1)
			return (split_backup(cut, &backup, line));
		if (aend == 0)
			return (end_of_file(line, &backup));
	}
}
